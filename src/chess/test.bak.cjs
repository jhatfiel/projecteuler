require('esm-hook');
const { Engine, State, Position, moveToString } = require('../../dist/out-tsc/chess/achess.js');
...
async function testPosition(caseNum, krkStr, solutionDepth, depth = 13) {
    ...
    let details = `${moves.map(move => moveToString(move))}`;
    if (STATE.evaluate() !== 1 || moves.length !== solutionDepth) {
        details = `FAILED ${originalResult.evaluation} / ${originalResult.depth} / ${moves.map(move => moveToString(move))} ${STATE.evaluate()}`;
    }
